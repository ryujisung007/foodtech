...
def render_results(filtered_df, full_df):
    ...
    if not filtered_df.empty:
        ...
        row = filtered_df[filtered_df['filtered_df']['기업이름'] == target_company].iloc[0]
        
        if st.button(f"🚀 {target_company} 신제품 제안 및 시각화"):
            with st.spinner("Gemini가 데이터를 분석하고 시각화하는 중..."):
...
def render_chatbot(df):
    ...
    if prompt := st.chat_input("질문 시 이전 대화는 자동 정리됩니다."):
