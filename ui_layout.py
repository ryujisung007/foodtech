...
def render_results(filtered_df):
    if not filtered_df.empty:
        ...
        st.dataframe(
            ...
            column_config={"사이트 주소": st.column_config.LinkColumn("사이트 링크")},
            hide_index=True
        )
        ...
        if st.button(f"🚀 {target_company} 기술 기반 제품 창작"):
            ...
            with st.spinner(f"{target_company}의 기술과 4대 카테고리를 융합 중입니다..."):
                ideas = engine_ai.get_product_ideation(row['기업이름'], row['대표기술'])
                st.markdown("---")
                st.success(f"### 📋 {target_company} 기술 융합 리포트")
                st.markdown(ideas)
    else:
        st.info("카테고리를 선택하면 상세 데이터가 표시됩니다.")import streamlit as st
import engine_ai
