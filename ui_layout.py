...
def render_sidebar(df, engine_data):
    ...
    # engine_data의 함수명과 정확히 매칭 (AttributeError 방지)
...
def render_results(filtered_df, full_df):
    ...
    if not filtered_df.empty:
        ...
        if st.button(f"🚀 {target_company} 신제품 제안 및 컨셉 시각화"):
            with st.spinner("Gemini가 데이터를 분석하여 시각화 중입니다..."):
                ...
                with col_right:
                    st.markdown("### 🖼️ 신소재 제품 컨셉 아트 (Imagen 3)")
                    # 실제 이미지 생성 API 연동 전까지 컨셉을 보여주는 시각화 영역
                    # URL 파라미터에 소재명을 넣어 가변적인 이미지를 유도
                    image_url = f"https://via.placeholder.com/800x600.png?text={target_company}+Food+Concept"
                    st.image(image_url, caption=f"{target_company} 소재 적용 컨셉", use_container_width=True)
                    st.success(f"**대상 소재:** {row['대표제품']}")
